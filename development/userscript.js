...
// @version      7.2
// @description  Floating automation buttons for Medinet KSK Forms (Hành Chính Trẻ Em, NCT, Người Lớn & Học Sinh Có Người Giám Hộ JSON AI OCR, Nhi Khoa, Tai, Răng Sơ Đồ FDI Native Sync, M4, Cận Lâm Sàng JSON Importer, Kết Luận)
// @author       Antigravity
...
(function () {
  ...
  /**
   * Unwrap JSON Payloads from LLM output (handles arrays, markdown codeblocks, and nested keys)
   */
  function unwrapJsonPayload(rawInput) {
    if (!rawInput) return {};
    let clean = String(rawInput).replace(/```json/gi, '').replace(/```/g, '').trim();
    let parsed;
    try {
  ...
  function setTextBoxValue(containerEl, valueStr) {
    ...
    // 1. Native human-typing event emulation on input element
    if (input) {
      input.focus();
    ...
    // 3. Direct sync hidden input for Angular reactive forms
    const hiddenInput = txtBoxEl.querySelector('input[type="hidden"]') || containerEl.querySelector('input[type="hidden"]');
    if (hiddenInput) {
  ...
  function setDateBoxValue(containerEl, dateStr) {
    ...
    if (formattedStr.includes('/')) {
      ...
      if (parts.length === 3) {
        ...
        if (p0 > 1000) { // yyyy/MM/dd
          dateObj = new Date(p0, p1 - 1, p2, 0, 0, 0);
        } else { // dd/MM/yyyy
          dateObj = new Date(p2, p1 - 1, p0, 0, 0, 0);
        }
      }
    ...
    const isoStr = `${yyyy}-${mm}-${dd}T00:00:00`;
    const displayStr = `${dd}/${mm}/${yyyy}`;

    // 1. DevExtreme widget option & validation sync
    ...
    // 2. Direct sync hidden input for Angular reactive forms (dateserializationformat="yyyy-MM-ddTHH:mm:ss")
    const hiddenInput = dateBoxEl.querySelector('input[type="hidden"]') || containerEl.querySelector('input[type="hidden"]');
    if (hiddenInput) {
    ...
    // 3. Update visible input element text
  ...
  function findMatchingDropdownItem(items, searchText, displayExpr) {
    if (!items || !Array.isArray(items) || !items.length) return null;

    const normalize = (str) => {
      return String(str || '')
        .toLowerCase()
        .replace(/^(phường|xã|thị trấn|tp\.|quận|huyện|tỉnh)\s+/i, '')
        .replace(/\s+/g, ' ')
        .trim();
    };
    ...
    for (let item of items) {
      const itemLabel = (typeof item === 'object' && displayExpr) ? item[displayExpr] : item;
      const cleanLabel = normalize(itemLabel);

      if (cleanLabel === cleanSearch || cleanLabel.includes(cleanSearch) || cleanSearch.includes(cleanLabel)) {
        return item;
      }
    }
    return null;
  ...
  function simulateDropdownSelect(target, searchText, retries = 5) {
    return new Promise(resolve => {
      ...
      // 1. Direct widget selection with valueExpr & hidden input sync
      if (widgetInst) {
        let items = widgetInst.option('items') || [];
        if (!items.length) {
          const ds = widgetInst.option('dataSource');
          if (Array.isArray(ds)) items = ds;
          else if (ds && ds.store) {
            const store = typeof ds.store === 'function' ? ds.store() : ds.store;
            if (store && store._array) items = store._array;
          }
        }

        const displayExpr = widgetInst.option('displayExpr') || 'text';
        const valueExpr = widgetInst.option('valueExpr') || 'id';

        const matchedItem = findMatchingDropdownItem(items, searchText, displayExpr);
        if (matchedItem) {
          const valToSet = (typeof matchedItem === 'object' && valueExpr && matchedItem[valueExpr] !== undefined)
            ? matchedItem[valueExpr]
            : matchedItem;

          try {
            if (widgetInst.NAME === 'dxTagBox') {
              let currentVal = widgetInst.option('value') || [];
              if (!Array.isArray(currentVal)) currentVal = [currentVal];
              if (!currentVal.includes(valToSet)) {
                widgetInst.option('value', [...currentVal, valToSet]);
              }
            } else {
              widgetInst.option('value', valToSet);
            }
            widgetInst.option('isValid', true);
          } catch (e) {}

          // Sync hidden input
          const hiddenInput = widget.querySelector('input[type="hidden"]') || targetContainer.querySelector('input[type="hidden"]');
          if (hiddenInput) {
            hiddenInput.value = String(valToSet);
            hiddenInput.dispatchEvent(new Event('change', { bubbles: true }));
            hiddenInput.dispatchEvent(new Event('input', { bubbles: true }));
          }

          const visibleInput = widget.querySelector('input.dx-texteditor-input');
          if (visibleInput && typeof matchedItem === 'object' && displayExpr) {
            visibleInput.value = String(matchedItem[displayExpr] || searchText);
          }

          widget.classList.remove('dx-invalid', 'dx-texteditor-empty');
          targetContainer.classList.remove('dx-invalid', 'dx-texteditor-empty');
          return resolve(true);
        }
      }

      // 2. Open dropdown overlay fallback
    ...
    if (radioGroup) {
      ...
      for (let i = 0; i < btns.length; i++) {
        ...
        if (text.includes(cleanOption) || cleanOption.includes(text)) {
          let valToSet = btn.textContent.trim();
          if (inst) {
            try {
              const items = inst.option('items');
              if (items && items[i] !== undefined) {
                const valueExpr = inst.option('valueExpr');
                valToSet = (typeof items[i] === 'object' && valueExpr && items[i][valueExpr] !== undefined)
                  ? items[i][valueExpr]
                  : items[i];
                inst.option('value', valToSet);
              } else {
                inst.option('value', valToSet);
              }
              inst.option('isValid', true);
          ...
          // Direct sync hidden input
          const hiddenInput = radioGroup.querySelector('input[type="hidden"]') || targetContainer.querySelector('input[type="hidden"]');
          if (hiddenInput) {
            hiddenInput.value = typeof valToSet === 'object' ? JSON.stringify(valToSet) : String(valToSet);
            hiddenInput.dispatchEvent(new Event('change', { bubbles: true }));
          }
  ...
  async function parseAndFillHanhChinhNCT(jsonInput) {
    let data;
    try {
      data = unwrapJsonPayload(jsonInput);
    } catch (e) {
    ...
    // 1. Số CCCD / CMND (Strict 12 digits)
    ...
    // 8. Xã / Phường (from data or parsed from dia_chi)
    ...
    // 9. Standard Form Defaults for Người Cao Tuổi
            ...
            <textarea id="jsonInputAreaNCT" placeholder='Vd:
{
  ...
  async function parseAndFillHocSinhGiamHo(jsonInput) {
    ...
    // 1. Số CCCD / Mã định danh học sinh (12 chữ số)
    ...
    // 2. Họ và tên học sinh (Uppercase)
    const hoTen = norm.hoten || norm.name || norm.fullname || norm.hovaten || data.ho_ten;
    if (hoTen) {
      const upperName = String(hoTen).trim().toUpperCase();
      const el = document.querySelector('.HoTen, [class*="HoTen"]');
    ...
    // 6. Số điện thoại học sinh / liên hệ
    const sdt = norm.sdt || norm.sodienthoai || norm.phone || norm.phonenumber || norm.dienthoaididong || data.sdt;
    if (sdt) {
      const cleanSdt = String(sdt).replace(/[^0-9+]/g, '');
      const el = document.querySelector('.SDT, [class*="SDT"]');
    ...
    // 7. Nơi ở hiện tại / Chỗ ở hiện tại
    const diaChi = norm.diachi || norm.noiohientai || norm.address || norm.currentplaceofliving || data.dia_chi;
    if (diaChi) {
      const el = document.querySelector('.DiaChiHienTai, [class*="DiaChiHienTai"]');
      if (el && setTextBoxValue(el, String(diaChi).trim())) count++;
    ...
    // 8. Xã / Phường nơi ở (từ JSON hoặc parse từ địa chỉ)
    let xaPhuong = norm.xaphuong || norm.ward || data.xa_phuong;
    if (!xaPhuong && diaChi) {
      const lower = diaChi.toLowerCase();
      if (lower.includes('lái thiêu')) xaPhuong = 'Lái Thiêu';
    ...
    // 9. Dân tộc (Default: "Kinh")
    const danToc = norm.dantoc || norm.ethnicity || 'Kinh';
    const danTocEl = document.querySelector('.DanTocId, [class*="DanTocId"]') || 'Dân tộc';
    await simulateDropdownSelect(danTocEl, danToc);
    count++;

    // 10. Lý do khám sức khỏe
    ...
    // 15. Người giám hộ (Họ và tên mẹ hoặc người giám hộ)
    ...
    // 16. CCCD người giám hộ (Strict 12 digits)
    ...
    // 17. Mối quan hệ với trẻ (Mẹ / Cha / Ông/bà / Anh/chị / Họ hàng / Khác)
    ...
    // 19. Form defaults (Đối tượng khám, Địa điểm khám, Hình thức chi trả, Hình thức khám)
    const dtEl = document.querySelector('.DoiTuong_M13, [class*="DoiTuong_M13"]');
    if (dtEl) {
...
LƯU Ý CỰC KỲ QUAN TRỌNG (YÊU CẦU ĐỘ CHÍNH XÁC TUYỆT ĐỐI TỪNG KÝ TỰ/CHỮ SỐ):
1. "ho_ten": Viết in hoa đầy đủ họ và tên học sinh có dấu (Mục 1).
2. "gioi_tinh": "Nam" hoặc "Nữ" (Mục 2).
3. "ngay_sinh": Ngày tháng năm sinh định dạng dd/mm/yyyy (Mục 3).
4. "cccd": Số CCCD/Mã định danh/Hộ chiếu của học sinh - Yêu cầu chính xác 100% các chữ số (Mục 6).
5. "bhyt": Số thẻ BHYT của học sinh - Yêu cầu chính xác 100% các chữ số/ký tự (Mục 7).
6. "dia_chi": Nơi ở hiện tại của học sinh - Ghi chính xác đầy đủ địa chỉ (Mục 8).
7. "xa_phuong": Tên Phường/Xã trích xuất từ nơi ở hoặc địa chỉ (nếu có).
8. "ten_truong": Tên trường học (Mục 10).
9. "lop": Lớp học của học sinh (nếu có, ví dụ: "2.2").
10. "nguoi_giam_ho": Họ và tên mẹ hoặc người giám hộ (Mục 12).
11. "cccd_giam_ho": Số CCCD của mẹ hoặc người giám hộ (Mục 12) - Yêu cầu chính xác 100% các chữ số.
12. "moi_quan_he": Mối quan hệ với trẻ (Mục 13) - Ví dụ: "Mẹ", "Cha", "Ông/bà",...
13. "sdt": Số điện thoại di động (Mục 14) - Chỉ lấy các chữ số, không được sai sót.

Chỉ trả về JSON thuần túy theo mẫu sau, không thêm bất kỳ lời giải thích nào:
{
  "ho_ten": "...",
  "gioi_tinh": "Nam/Nữ",
  "ngay_sinh": "dd/mm/yyyy",
            ...
            <textarea id="jsonInputAreaHSGH" placeholder='Vd:
{
  ...
  async function parseAndFillNguoiLon(jsonInput) {
    ...
    // 1. Số CCCD / Mã định danh / Hộ chiếu (12 chữ số) - VITAL
    const cccd = norm.cccd || norm.cmnd || norm.socccd || norm.idcard || norm.dinhdanh || norm.cccdidnumber || data.cccd;
    if (cccd) {
      let cleanCccd = String(cccd).replace(/[^0-9]/g, '');
      if (cleanCccd.length === 11) cleanCccd = cleanCccd.padStart(12, '0');
    ...
    // 2. Họ và tên (Uppercase có dấu) - VITAL
    const hoTen = norm.hoten || norm.name || norm.fullname || norm.hovaten || data.ho_ten;
    if (hoTen) {
      const upperName = String(hoTen).trim().toUpperCase();
      const el = document.querySelector('.HoTen, [class*="HoTen"]');
    ...
    // 4. Ngày tháng năm sinh (dd/MM/yyyy) - VITAL
    const ngaySinh = norm.ngaysinh || norm.dateofbirth || norm.dob || norm.birthdate || norm.birth_date || norm.ngaythangnamsinh || norm.ngay || data.ngay_sinh;
    if (ngaySinh) {
      const el = document.querySelector('.NgaySinh, [class*="NgaySinh"]');
      if (el && setDateBoxValue(el, String(ngaySinh))) count++;
    ...
    // 7. Nơi ở hiện tại / Địa chỉ - VITAL
    const diaChi = norm.diachi || norm.noiohientai || norm.address || norm.currentplaceofliving || data.dia_chi;
    if (diaChi) {
      const el = document.querySelector('.DiaChiHienTai, [class*="DiaChiHienTai"]');
      if (el && setTextBoxValue(el, String(diaChi).trim())) count++;
    ...
    // 8. Xã / Phường nơi ở (từ JSON hoặc parse từ địa chỉ) - VITAL
    let xaPhuong = norm.xaphuong || norm.ward || data.xa_phuong;
    if (!xaPhuong && diaChi) {
      const lower = diaChi.toLowerCase();
      if (lower.includes('lái thiêu')) xaPhuong = 'Lái Thiêu';
    ...
    // 13. Form defaults (Đối tượng khám, Địa điểm khám, Hình thức chi trả, Hình thức khám)
    const dtEl = document.querySelector('.DoiTuong_M13, [class*="DoiTuong_M13"]');
    if (dtEl) {
...
LƯU Ý CỰC KỲ QUAN TRỌNG (YÊU CẦU ĐỘ CHÍNH XÁC TUYỆT ĐỐI TỪNG KÝ TỰ/CHỮ SỐ):
1. "ho_ten": Viết in hoa đầy đủ họ và tên có dấu (Mục 1). Ví dụ: "NGUYỄN THỊ HUYỀN".
2. "gioi_tinh": "Nam" hoặc "Nữ" (Mục 2).
3. "ngay_sinh": Ngày tháng năm sinh định dạng dd/mm/yyyy (Mục 3). Ví dụ: "20/01/1982".
4. "dan_toc": Ghi rõ dân tộc (Mục 4, mặc định "Kinh" nếu để trống).
5. "cccd": Số CCCD/Mã định danh/Hộ chiếu (Mục 6) - Yêu cầu chính xác 100% đủ 12 chữ số.
6. "bhyt": Số thẻ BHYT (Mục 7, để trống nếu không có).
7. "dia_chi": Nơi ở hiện tại - Ghi chính xác đầy đủ địa chỉ (Mục 8). Ví dụ: "A4089, Bình Đức, Lái Thiêu".
8. "xa_phuong": Tên Phường/Xã trích xuất từ nơi ở hoặc địa chỉ (Ví dụ: "Lái Thiêu", "An Phú", "Thuận Giao", "Bình Chuẩn",...).
9. "nghe_nghiep": Nghề nghiệp (Mục 9). Ví dụ: "Lao động tự do".
10. "noi_lam_viec": Nơi làm việc, học tập (Mục 10, để trống nếu không có).
11. "sdt": Số điện thoại di động (Mục 11) - Chỉ lấy các chữ số, tuyệt đối không được sai sót.
12. "ly_do_kham": Lý do khám sức khỏe (Mục 12, mặc định "Khám sức khỏe định kỳ").
            ...
            <textarea id="jsonInputAreaNL" placeholder='Vd:
{
    ...
    try {
      ...
      for (let item of items) {
        ...
        if (itemText && String(itemText).toLowerCase().includes(textToFind.toLowerCase())) {
          const valToSet = (typeof item === 'object' && valueExpr && item[valueExpr] !== undefined) ? item[valueExpr] : item;
  ...
  function findToothElement(toothNumber, rootDoc = document) {
    ...
    // 1. Check native Medinet dental chart select by data-tooth or ID
    ...
    // 3. Fallback: DevExtreme select box / tagbox
...
Chỉ trả về JSON thuần túy, không thêm lời giải thích.`;
  ...
  function injectButtons() {
    ...
    toggleBtn.textContent = '⚙️ SKCD Tools v7.2';
